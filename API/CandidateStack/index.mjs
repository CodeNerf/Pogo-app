...
export const handler = async (event, context) => {
  ...
  try {
    switch (event.routeKey) {
      case "DELETE /userCandidateStack/{userId}":
        await dynamo.send(
          new DeleteCommand({
            TableName: tableName,
            Key: {
        ...
        break;
      case "GET /userCandidateStack/{userId}":
        ...
        break;
      case "GET /userCandidateStack/demographics/{userId}":
        ...
        let candidateDemographics = await dynamo.send(
          new ScanCommand({ TableName: "CandidateDemographics" })
        );
        ...
        body = candidateList;
        break;
      case "GET /userCandidateStack/issues/{userId}":
        stack = await dynamo.send(
          new GetCommand({
            TableName: tableName,
            Key: {
